...
function TransactionSection({ title, status, isExpanded = false, onToggle, children, lockMessage, isHighlighted = false, sectionRef }: SectionProps) {
  ...
  return (
    <div
      ref={sectionRef}
      className={`overflow-hidden border rounded-lg shadow-sm transition-all duration-500 ${isHighlighted
    ...
    >
      <button
        ...
        className={`flex items-center justify-between w-full px-4 sm:px-6 py-3 sm:py-4 text-left transition-colors ${status === 'locked'
      ...
      <div className={`transition-all duration-300 ease-in-out ${isExpanded && status !== 'locked' ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'
        } overflow-hidden`}>
        ...
        <div className="px-4 sm:px-6 py-4">
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <button
                onClick={() => window.history.back()}
                className="flex items-center gap-2 p-2 text-red-600 rounded-lg
  hover:bg-gray-100"
              >
          ...
          <div className="flex-1 min-w-0 space-y-4">
            ...
            >
              <div className="space-y-6">
                                ...
                                <div key={idx} className="flex items-center justify-between p-2 bg-cyan-50 border border-cyan-200 rounded text-xs">
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                      ...
                                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${po.status === 'sent' ? 'bg-blue-100 text-blue-700' :
                                  ...
                                  {(() => {
                                    ...
                                    return (
                                      <>
                                        ...
                                        <div
                                          className={`px-2 py-1 rounded-full text-xs font-medium ${lineItemQuotes.length > 0
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded text-xs font-medium ${rfq.status === 'sent'
                ...
                {/* RFQ Workflow: Client Quotation Summary */}
                          ...
                          {clientQuotes.map((quote, idx) => (
                            <div key={idx} className="p-3 bg-white border border-blue-200 rounded">
                              <div className="flex items-start justify-between mb-2">
                                ...
                                <span
                                  className={`px-2 py-1 rounded text-xs font-medium ${quote.status === 'sent'
                                    ? 'bg-blue-100 text-blue-700'
                                ...
                                {quote.status === 'sent' && (
                                  <>
                                    <button
                                      ...
                                      className={`text-xs hover:underline ${hasAcceptedQuote()
                ...
                {/* CLIENT_PO Workflow: Direct PO Acceptance */}
                {transaction.transactionType === 'client_po' && (
                  <div className="p-4 border-2 border-green-200 rounded-lg bg-green-50">
                    <h4 className="mb-3 text-sm font-semibold text-green-900">Client Purchase Order - Direct Processing</h4>
                    <p className="mb-3 text-xs text-gray-600">
                      This is a CLIENT_PO transaction. The client has already committed to purchase via their PO.
                      Quote generation and sending steps are skipped.
                    </p>

                    {/* Cost Summary */}
                    <div className="p-3 mb-3 space-y-2 text-sm bg-white border border-green-200 rounded">
                      {(() => {
                        const stockItems = transaction?.lineItems?.filter(item => !item.requiresSourcing) || [];
                        const sourcingItems = transaction?.lineItems?.filter(item => item.requiresSourcing) || [];
                        ...
                        const totalCost = stockCost + sourcingCost;
                        const allItemsCosted = transaction.lineItems.every(item =>
                          item.costingStrategy !== 'pending'
                        );
                            ...
                            {!allItemsCosted && (
                              <div className="pt-2 mt-2 border-t border-yellow-200">
                                <p className="text-xs text-yellow-700">
                                  ⚠️ Some items are not yet fully costed. Complete sourcing to enable PO acceptance.
                                </p>
                              </div>
                            )}
                          </>
                    ...
                    {(() => {
                      ...
                      const isAccepted = transaction.status === 'accepted' ||
                                        ...
                                        transaction.status === 'waiting_for_items' ||
                                        transaction.status === 'ready_for_delivery' ||
                                        transaction.status === 'out_for_delivery' ||
                      ...
                      return (
                        <button
                          onClick={async () => {
                            try {
                              await api.post(`/transactions/${transaction.id}/accept-client-po`);
                              showSuccess('Client PO accepted! Processing allocation...');

                              // Trigger allocation
                              await api.post(`/transactions/${transaction.id}/accept-po-and-allocate`);
                              showSuccess('Allocation processed successfully!');
                            } catch (error: any) {
                              console.error('Failed to accept PO:', error);
                              const errorMessage = error?.response?.data?.message || 'Failed to accept PO';
                              showError(errorMessage);
                            }
                          }}
                          disabled={!allItemsCosted || isAccepted}
                          className="flex items-center justify-center w-full gap-2 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
                          title={
                            isAccepted
                              ? 'PO has already been accepted'
                              : !allItemsCosted
                                ? 'All items must be costed before accepting PO'
                                : 'Accept Client PO and process allocation directly'
                          }
                        >
                          {isAccepted ? 'PO Already Accepted' : 'Accept Client PO & Allocate'}
                        </button>
                      );
                    })()}
                      ...
                      if (isAccepted) {
                        return (
                          <div className="mt-3 p-2 bg-green-100 border border-green-300 rounded text-xs text-green-800">
                            ✓ Client PO has been accepted. Proceed to fulfillment section.
                          </div>
                        );
                      }
                      return null;
